...
class TNode{
    ...
    public:
        ...
        std::string createChild(int childId, int player_count){
            int port;
            bool isleft = false;
            if(leftId == -2){
            ...
            int pid = fork();
            if(pid == 0){
                execl("./game", "game", std::to_string(childId).c_str(), std::to_string(port).c_str(), std::to_string(id).c_str(), std::to_string(player_count).c_str(), (char*)NULL);
        ...
        std::string game(std::string str, std::string number){
            int bull = 0, cow = 0;
            for (int i = 0; i < 4; ++i) {
                if (str[i] == number[i]) {
                    ++bull;
            ...
            std::string answer = std::to_string(bull) + " быка и " + std::to_string(cow) + " коровы!";
            return answer;
        }
